...
\begin{document}
\title{\Large{Untersuchung und Implementierung von Graph-Metriken und deren Anwendung auf Graph-Isomorphie
}}
...
\begin{titlepage}
	...
	Untersuchung und Implementierung von Graph-Metriken und deren Anwendung auf Graph-Isomorphie
	}
	\\
	\vspace{2cm}
...
Diese Studienarbeit soll nun diese Metriken genauer untersuchen. Hierbei ist es zunächst wichtig die verschiedenste Metriken vorzustellen und zu erläutern. Dabei ist es auch relevant herauszufinden, wie verbreitet diese Metriken sind und inwieweit die jeweiligen Kennzahlen im Bezug auf ihre Berechenbarkeit zu bewerten sind. Des Weiteren soll auf Basis der Metriken auch der Begriff der Isomorphie bzw. Ähnlichkeit von Graphen behandelt werden.

Neben einer theoretischen Betrachtung der Graphmetriken soll auch eine Implementierung bzw. Nutzung dieser stattfinden. Es ist dabei das Ziel, mithilfe von Graphdatenbanken die jeweiligen Metriken umzusetzen und diese miteinander zu Vergleichen.

In einem Weiteren Teil ist außerdem noch darauf einzugehen, welche Anwendung die gezeigten Metriken haben, um den praktischen Nutzen der Thematik aufzuzeigen. Es soll gezeigt werden, wie die angesprochenen Thematiken an realen Problemen Anwendung finden.
}
...
\singleenum{2}{Inwiefern lassen sich Graph-Metriken dafür verwenden, die Isomorphie zweier Graphen zu ermitteln und wie lässt sich das an einem Anwendungsbeispiel verdeutlichen?}

Basierend auf Graph-Metriken lässt sich auch ermitteln, ob zwei Graphen Ähnlichkeiten aufweißen bzw. isomorph sind \cite{wills_metrics_2019}. Auch auf diesen Aspekt soll die Arbeit bezug nehmen und dabei ein Anwendungs-Beispiel konstruieren.
...
\singleenum{5}{Welche realen Anwendungen gibt es für Graph-Metriken und den Vergleich von Graphen (Ähnlichkeit/Isomorphie)?}
...
\metric{Fraktionierte chromatische Zahl}{Statt einem Knoten nur eine Farbe zuzuweisen können ihm auch \(b\) Farben zugewiesen werden. \(\chi_b(G)\) gibt die minimale Zahl an unterschiedlichen Farben an, für die \(G\) so färbbar ist, das zwei adjazente Knoten in ihren Farben keine Schnittmenge haben. Die fraktionierte chromatische Zahl ist auf Basis dessen so formuliert:
\[\chi_f(G)=\lim_{b\to\infty}\frac{\chi_b(G)}{b}\]
}
