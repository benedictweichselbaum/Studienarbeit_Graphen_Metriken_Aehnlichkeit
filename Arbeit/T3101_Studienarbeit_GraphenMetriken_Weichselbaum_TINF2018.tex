...
\begin{document}
\title{\Large{Untersuchung und Umsetzung von Graphmetriken
}}
...
\begin{titlepage}
	...
	Untersuchung und Umsetzung von Graphmetriken
	}
	\\
	\vspace{2cm}
...
\section*{Erklärung}
\thispagestyle{empty}
Ich versichere hiermit, dass ich meine Studienarbeit mit dem Thema \textit{Untersuchung und Umsetzung von Graphmetriken} selbstständig verfasst und keine anderen als die angegebenen Quellen und Hilfsmittel benutzt habe. \vspace{1.6cm}\newline
...
\singleenum{0}{Welche Graphmetriken gibt es und wie sind diese definiert, zu berechnen und zu kategorisieren?}
...
\chapter{Graphmetriken}
{
Der erste Teil der Arbeit wird sich ausführlich mit einer umfassenden Menge an Graphmetriken beschäftigen. Hierbei sollen die ersten zwei Fragestellungen der Arbeit genau beantwortet werden. Um dies zu bewerkstelligen, werden die jeweiligen recherchierten Metriken unter verschieden Gesichtspunkten beleuchtet. Zur jeweiligen Vorstellung einer Graphmetrik sollen dabei die folgengen Punkte erläutert werden:
\begin{myitemize}
...
\metricnoline{Abstand/Distanz \(d_G(x,y)\)}{Die Länge des kürzesten Weges zwischen den beiden Knoten \(x\) und \(y\).}
...
\subsection*{Kreis-basierte Metriken}
...
\subsection*{Zusammenhangsmetriken}
...
\subsection*{Zentralitätsmetriken}
...
\metricnoline{Eigenvektor Centrality \(x_v\)}{Zentralitätsmetrik, die einen Knoten dahingehend bewertet, wie vernetzt er ist und wie vernetzt seine Nachbarn sind. Wendet man die Metrik auf die gesamte Adjazenzmatrix \(A\) an, so sind die Eigenvektor Centralities aller Knoten im Eigenvektor \(x\) zu finden, der nur positive Werte enthält.
\[
	x_v=\frac{1}{\lambda}\sum_{t\in M(v)}x_t=\frac{1}{\lambda}\sum_{t\in G}a_{v,t}x_t
\]
}
...
\subsection*{Chromatische Zahl und chromatischer Index}
...
Ein Graph dient meist zur Repräsentation und Abstraktion eines bestimmten Sachverhaltes \cite{turau_algorithmische_2004}. Viele Beispiele dazu konnten z.B. in Kapitel 2 gesehen werden, wobei unter anderem Anwendungen für Metriken besprochen und vorgestellt wurden. Auch die Graphdatenstruktur muss es ermöglichen, dass eine gewünschte Abstraktion dargestellt werden. Die Bibliothek muss es also ermöglichen Knoten und Kanten im Graphen zu "`markieren"'. Durch die Markierung muss es zudem möglich werden, die Knoten eindeutig zu identifizieren. Diese eindeutige Identifikation dient anschließend auch zur Durchführung der diversen Aktionen, die auf den Graphen möglich sind. Die verwendete Markierung soll dabei beliebig sein. Allerdings ist es notwendig, dass Knotenmarkierungen untereinander vergleichbar sind, damit die Funktionalität des Graphen gewährleistet werden kann.
...
Zur Wahl der Datenstruktur muss zunächst betrachtet werden, welche Möglichkeiten es gibt, einen Graphen zu repräsentieren, wenn man ihn programmtechnisch im Arbeitsspeicher darstellt. Grundsätzlich lassen sich vier Datenstrukturen ausmachen \cite{knebl_algorithmen_2019}:
\begin{myitemize}
	\item Adjazenzmatrix
	\item Adjazenzlisten
	\item Inzidenzmatrix
...
\begin{myitemize}
	\item \textbf{BasicGraphMetricCalculation}: Berechnung grundlegender Graphmetriken (Ordnung, Größe etc.)
...
\section{Algorithmik und Implementierung der Graphmetriken}
{
...
\geintrag{Cypher}{Abfragesprache für die neo4j-Graphdatenbank. \cite{kollegger_gram_2021}}
\geintrag{Graph}{Ein Paar bestehend auf Knoten\(V\)und Kanten E. \(G=(V,E), \text{wobei }E\subseteq V\times V\)}
\geintrag{Graphinvariante}{Eine Funktion, die einen Graphen auf eine Zahl abbildet. Sind zwei Graphen isomorph so wird ihnen der gleiche Wert zugewiesen. \cite{diestel_graphentheorie_2000}}
\geintrag{Graphmetrik}{siehe \textit{Graphinvariante}}
...
\geintrag{neo4j}{Java-basierende Graphdatenbank. Website: \url{https://neo4j.com/}}
...
\geintrag{Web-Socket}{Protokoll, das es erlaubt eine persistente Verbindung zwischen zwei Computern aufzubauen, damit diese jederzeit Daten miteinander austauschen können. \cite{mozilla_websockets_2021}}
