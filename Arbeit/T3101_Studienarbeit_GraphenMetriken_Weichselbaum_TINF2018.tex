...
\singleenum{2}{Welche Anforderungen müssen an eine Klassenbibliothek zur Implementierung der vorgestellten Metriken gestellt werden?}

Im darauffolgenden parktischen Teil der Arbeit sollen die recherchierten Metriken im Rahmen einer Klassenbibliothek implementiert werden. Ziel ist es, das erlangte theoretische Wissen praktisch anzuwenden. Hierbei ist es zunächst wichtig die jeweiligen Anforderungen an die zu erstellende Software zu ermitteln. Zu diesen Anforderungen gehören sowohl funktionale als auch nicht funktionale Eigenschaften. 

\singleenum{3}{Wie können die Anforderungen der Klassenbibliothek umgesetzt und getestet werden?}
...
\singleenum{4}{Wie wurden die konkreten Algorithmen zur Berechnung der Graphmetriken implementiert?}

Die Kernaufgabe der Bibliothek ist es, für einen vorgegebenen Graphen diverse Metriken zu berechnen. Hierfür ist eine Reihe von verschiedenen Algorithmen notwendig. Aufgrund dessen ist es notwenig, dass diese und ihr Zusammenspiel im Rahmen der Entwicklung erläutert werden.

\singleenum{5}{Wie kann die Berechnung der Graphmetriken weiter abstrahiert werden?}
...
\singleenum{5}{Wie könnte ein Ähnlichkeitsmaß zum Vergleich zweier Graphen konzeptioniert sein?}
...
\singleenum{6}{Wie lässt sich das erarbeitete Ähnlichkeitsmaß implementieren und umsetzen?}
...
\singleenum{7}{Wie ist das implementierte Ähnlichkeitsmaß zum Vergleich zweier Graphen im Hinblick auf seine Genauigkeit zu bewerten?}

Anschließend ist es wichtig festzuhalten, wie hoch die Qualität des erstellten Ähnlich-keitsmaßes ist. Hierfür soll die Arbeit unter Einbezug von Beispielen erproben, wie gut die Ähnlichkeit zwischen zwei Graphen erkannt wird.
}
...
Inzidenzliste- und Matrix repräsentieren den Graphen durch das aneinanderreihen von Kanten und ihren Informationen (inzidente Knoten). Dadurch sind Kanteneinfüge- und löschoperationen leicht zu realisieren. Allerdings ist man bei der Matrix durch ihre festgelegte Größte stärker beschränkt und muss beim Löschen und Einfügen die Matrix nur initialisieren. Der Speicherbedarf steigt linear bei beiden Lösungen mit der Anzahl der Kanten. Bei der Inzidenzmatrix wird die Matrix zudem linear größer, desto mehr Knoten im Graphen sind.
